...
public class BuyerSteps extends Steps {
    ...
    private String checkRegion(int buyerID) throws SQLException {
        ...
        String expected = Context.getTestContext().getJsonObject().getAsJsonObject("billing_address").get("state").toString().replace("\"", "");
    ...
    private String checkPhoneNumber(int buyerID) throws SQLException {
        ...
        String expected = Context.getTestContext().getJsonObject().getAsJsonObject("shipping_address").get("phone_1").toString().replace("\"", "");
        return verifyExpectedResults(actual, expected);
    }
    ...
    private String checkPostalCode(int buyerID) throws SQLException {
        ...
        String expected = Context.getTestContext().getJsonObject().getAsJsonObject("billing_address").get("zip").toString().replace("\"", "");
        return verifyExpectedResults(actual, expected);
    }
